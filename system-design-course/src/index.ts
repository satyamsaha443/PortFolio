...
import type { Course, Module, Lesson } from "./types/course.js";
...
function loadCourseIndex(): Course {
  ...
  return JSON.parse(readFileSync(INDEX_FILE, "utf-8")) as Course;
}
...
function renderLesson(lesson: Lesson): void {
  ...
  console.log(marked(md) as string);
}

function printModuleHeader(mod: Module): void {
...
function printTableOfContents(course: Course): void {
...
program
  ...
  .action((moduleNum: string, lessonNum: string | undefined) => {
    const course = loadCourseIndex();
