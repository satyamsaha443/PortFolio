...
Scaling Memcache at Facebook — Nishtala et al. (NSDI 2013) — the canonical look-aside cache at scale: leases for stale-set/thundering-herd, regional pools, and cross-region invalidation.

Optimal Probabilistic Cache Stampede Prevention — Vattani, Chierichetti & Lowenstein (VLDB 2015) — the probabilistic early-expiration rule for spreading refresh off the TTL boundary.

TinyLFU: A Highly Efficient Cache Admission Policy — Einziger, Friedman & Manes — the frequency-sketch admission filter behind W-TinyLFU's scan resistance.

Caching patterns (read-through / write-through / write-behind / refresh-ahead) — Oracle Coherence docs — precise definitions of the write-path strategies.

Consistent hashing and libketama — metabrew — virtual nodes and the ring used by Memcached clients.
